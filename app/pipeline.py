...
from .symbols import resolve_instrument, resolve_screener_ticker
...
def run_foundation(portfolio_path, screener_path, ledger_path, as_of=None, run_id=None):
    ...
    # Canonicalize only through the explicit screener-to-canonical crosswalk,
    # while preserving conservative normalization for every other ticker.
    screener_by_ticker = {}
    for row in screener_rows:
        resolution = resolve_screener_ticker(row["ticker"])
        if resolution.ticker:
            screener_by_ticker[resolution.ticker] = row
